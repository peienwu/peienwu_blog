...
用到**DFS剪枝**，[筆記在這](https://hackmd.io/@peienwu/H1eA1ilHO)
```cpp=
#include <bits/stdc++.h>
using namespace std;
...
這一題用DFS明顯比BFS來得好、**空間**省很多（用一下資芽的圖:sweat_smile:)
...
除此之外，我因為邊界問題（$10^8$ 或是$10^8+1$）沒有仔細注意就一直WA :cry:
...
大開眼界！（不過只是用於整數排序）
這次手寫覺得有些困難，不過學到了很多東西！（可以來做一個各種排序演算的筆記:+1:）